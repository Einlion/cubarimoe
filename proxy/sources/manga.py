...
class Manga(ProxySource):
    mappings = {
        "The-Classroom-of-a-Black-Cat-and-a-Witch": "Z2lzdC9FaW5saW9uLzgyMWRhMGNmZmVkZTVhYjIzZDk0ODFmMzRiNmU4YmI3L3Jhdy8"
    }
    ...
    async def series_api_handler(self, series_slug):
        meta_id = self.mappings[series_slug]
    ...
    async def chapter_api_handler(self, series_slug):
        pass

    @api_cache(prefix="gist_page_dt", time=300)
    async def series_page_handler(self, series_slug):
        meta_id = self.mappings[series_slug]
        data = await self.gist_common(meta_id)
        if data:
            return SeriesPage(
